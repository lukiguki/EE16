        ...
        <main>
            <section>
                <form action="#" method="post">
                    ...
                    <input name="email" type="text" class="form" value="<?php echo isset($_POST['email']) ? $_POST['email'] : '' ?>"><br>
...
if (isset($_POST["email"]) && isset($_POST["losenord"])){
    ...
    if (!$result) {
        ...
        if (password_verify($losen, $user['losen'])) {
            echo "<p class='animated bounceInUp greenbox'>Du är inloggad!</p>";
            $_SESSION['loggedin'] = true;
            $_SESSION['anamn'] = $user['fornamn'];
            header("Location: index.php");
        } else {
            echo "<p class='animated bounceInUp redbox'>Lösenordet är fel, var god och försök igen!</p>";
            $_SESSION['loggedin'] = false;
        }
